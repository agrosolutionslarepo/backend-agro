...
class EmpresaService {
  ...
  public async getNombreEmpresa(id: String): Promise<String> {
    const cleanId = sanitize(id as any) as string;
  ...
  public async getEmpresaById(id: string | Types.ObjectId) {
    const cleanId = sanitize(id as any) as string | Types.ObjectId;
    const empresa = await Empresa.findById(cleanId);
    return empresa;
  }
  ...
  public async crearEmpresaDesdeGoogle(nombreEmpresa: string, userId: string): Promise<IEmpresa> {
    const cleanName = sanitize(nombreEmpresa) as string;
    const cleanUserId = sanitize(userId) as string;
    const yaExiste = await Empresa.exists({ nombreEmpresa: cleanName });
    if (yaExiste) {
